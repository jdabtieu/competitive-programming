...
public class ccc03s3 {
    ...
    public static void main(String[] args) throws IOException {
        int floors = readInt(), height = readInt(), width = readInt();
        
        visited = new boolean[height][width];

        for (int i = 0; i < height; i++) {
            String s = readLine();
            for (int j = 0; j < width; j++) visited[i][j] = s.charAt(j) == 'I';
        }

        for (int i = 0; i < height; i++) for (int j = 0; j < width; j++) if (!visited[i][j]) bfs(i, j);
        
        int x = 0;
        Collections.sort(rooms, Collections.reverseOrder());
        for (int i = 0; i < rooms.size(); i++) {
            if (floors >= rooms.get(i)) {
                x++;
                floors -= rooms.get(i);
            } else break;
        }
        if (x == 1) System.out.printf("1 room, %d square metre(s) left over", floors);
        else System.out.printf("%d rooms, %d square metre(s) left over", x, floors);
    }
