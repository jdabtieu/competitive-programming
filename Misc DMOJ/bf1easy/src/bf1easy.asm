...
_read_buf:                      ; byte _read_buf()
    ...
    mov rdx, 4096               ; read up to 4096 characters
    syscall
...
write_out:                      ; void write_out(char *buf, int count) --> rdi, esi
    ...
    sub rsp, 8                  ; align stack to keep 8 bytes for buffering
    cmp esi, 4096
    jge _write_out_large        ; check if count is greater than 4096
    call _write_buf             ; call _write_buf if count < 4096
    jmp _write_out_done
_write_out_large:
...
_write_buf:                     ; void _write_buf(char *buf, int count) --> rdi, esi
    ; warning: only use _write_buf if count is guaranteed to be < 4096 bytes
...
section .bss
input_buf:     resb 4096        ; reserve 4096 bytes
output_buf:    resb 4096        ; reserve 4096 bytes

section .data
ibuf_start:    dd 4096          ; index 4096
ibuf_end:      dd 4096          ; index 4096
