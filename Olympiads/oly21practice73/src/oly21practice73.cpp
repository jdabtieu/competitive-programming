...
int main() {
    ...
    ll ans = 0;
    if (n % 2 == 0) {
        for (int i = n - 1; i > n / 2; i--) ans += 2 * x[i];
        for (int i = 0; i < n / 2 - 1; i++) ans -= 2 * x[i];
        ans -= x[n / 2 - 1];
        ans += x[n / 2];
    } else {
        for (int i = n - 1; i > n / 2 + 1; i--) ans += 2 * x[i];
        for (int i = 0; i < n / 2 - 1; i++) ans -= 2 * x[i];
        ll one = 2 * x[n / 2 + 1] - x[n / 2] - x[n / 2 - 1];
        ll two = -2 * x[n / 2 - 1] + x[n / 2] + x[n / 2 + 1];
        ans += max(one, two);
    }
    cout << ans;
}
