...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        int b, c;
        cin >> a >> b >> c;
        v.pb(tuple<string, int, int>(a, b, c));
    }
    int imax = 2000, imin = -2000;
    for (int z = n - 1; z >= 0; z--) {
        auto &it = v[z];
        if (get<0>(it).compare("on") == 0) {
            imin = max(imin - get<2>(it), 0);
            imax = max(imax - get<1>(it), 0);
        } else if (get<0>(it).compare("off") == 0) {
            imin += get<1>(it);
            imax += get<2>(it);
        } else {
            imax = min(imax, get<2>(it));
            imin = max(max(imin, get<1>(it)), 0);
        }
    }
    printf("%d %d\n", imin, imax);
