    ...
    public static void main(String[] args) throws IOException {
        for (int cases = readInt(); cases > 0; cases--) {
            ...
            while (!queue.isEmpty()) {
                ...
                if (shortest[x][y] >= 60) break;
                if (y - 1 >= 0 && grid[x][y - 1] == 0 && shortest[x][y - 1] == 0) {
                    queue.add(new Coord(x, y - 1));
                    shortest[x][y - 1] = shortest[x][y] + 1;
                }
                if (x + 1 < cols && grid[x + 1][y] == 0 && shortest[x + 1][y] == 0) {
                    queue.add(new Coord(x + 1, y));
                    shortest[x + 1][y] = shortest[x][y] + 1;
                }
                if (y + 1 < rows && grid[x][y + 1] == 0 && shortest[x][y + 1] == 0) {
                    queue.add(new Coord(x, y + 1));
                    shortest[x][y + 1] = shortest[x][y] + 1;
                }
                if (x - 1 >= 0 && grid[x - 1][y] == 0 && shortest[x - 1][y] == 0) {
                    queue.add(new Coord(x - 1, y));
                    shortest[x - 1][y] = shortest[x][y] + 1;
                }
            }
            if (shortest[end.getX()][end.getY()] < 60 && shortest[end.getX()][end.getY()] != 0) {
