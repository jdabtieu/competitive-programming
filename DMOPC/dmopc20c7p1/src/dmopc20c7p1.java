...
public class dmopc20c7p1 {
    ...
    public static void main(String[] args) throws IOException {
        Coord.setCompareMode("X");
        int n = readInt();
        ArrayList<Coord> sol = new ArrayList();
        ArrayList<Coord> weights = new ArrayList();
        for (int i = 1; i <= n*2; i++) {
            int x = readInt();
            weights.add(new Coord(x, i));
        }
        Collections.sort(weights);
        int s = 0;
        for (int i = 0; i < n; i++) {
            sol.add(new Coord(weights.get(i).getY(), weights.get(i + n).getY()));
            if (weights.get(i).getX() != weights.get(i + n).getX()) {
                s++;
            }
        }
